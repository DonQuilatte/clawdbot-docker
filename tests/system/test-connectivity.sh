...
# Detect best IP for main Mac (Tailscale preferred, LAN fallback)
MAIN_MAC_TAILSCALE_IP=$(tailscale ip -4 2>/dev/null || echo "")
MAIN_MAC_LAN_IP="${GATEWAY_IP:-${MAIN_MAC_IP:-192.168.1.230}}"

# Use Tailscale if available, otherwise LAN
if [[ -n "$MAIN_MAC_TAILSCALE_IP" ]]; then
    MAIN_MAC_IP="$MAIN_MAC_TAILSCALE_IP"
    NETWORK_TYPE="Tailscale"
else
    MAIN_MAC_IP="$MAIN_MAC_LAN_IP"
    NETWORK_TYPE="LAN"
fi

echo "Network: $NETWORK_TYPE ($MAIN_MAC_IP)"
...
# Test: Remote Mac can reach gateway (try Tailscale first, then LAN)
print_test "Remote Mac can reach gateway"
REMOTE_HEALTH="000"
USED_ROUTE=""

# Try Tailscale IP first (more reliable across network boundaries)
if [[ -n "$MAIN_MAC_TAILSCALE_IP" ]]; then
    REMOTE_HEALTH=$(ssh -o ConnectTimeout=5 -o BatchMode=yes "$REMOTE_USER@$REMOTE_MAC_IP" \
        "curl -s --connect-timeout 5 -o /dev/null -w '%{http_code}' http://$MAIN_MAC_TAILSCALE_IP:$GATEWAY_PORT/" 2>/dev/null || echo "000")
    [[ "$REMOTE_HEALTH" == "200" ]] && USED_ROUTE="Tailscale"
fi

# Fallback to LAN if Tailscale failed
if [[ "$REMOTE_HEALTH" != "200" && -n "$MAIN_MAC_LAN_IP" ]]; then
    REMOTE_HEALTH=$(ssh -o ConnectTimeout=5 -o BatchMode=yes "$REMOTE_USER@$REMOTE_MAC_IP" \
        "curl -s --connect-timeout 5 -o /dev/null -w '%{http_code}' http://$MAIN_MAC_LAN_IP:$GATEWAY_PORT/" 2>/dev/null || echo "000")
    [[ "$REMOTE_HEALTH" == "200" ]] && USED_ROUTE="LAN"
fi

if [[ "$REMOTE_HEALTH" == "200" ]]; then
    print_pass "Remote Mac can reach gateway via $USED_ROUTE (HTTP 200)"
else
